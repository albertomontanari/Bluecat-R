...
{
  #Beware: resultcalib must be a list, with elements qsim and qoss
  nstep=length(dmodelsim)
  ...
  aux=sort(dmodelsim,index.return=T)$ix
  #sortsim contains the simulated data in ascending order
  sortsim=sort(dmodelsim)
  ...
  if(empquant==F)
    ...
    const=min(c(min(medpred),min(qossc),min(sortcalibsim),min(dmodelsim)))
    if (const<0)
      {
  ...
  if(plotflag==T && nstep1>20 && is.null(qoss)==F)
    ...
    sortdata=sort(dmodelsim[qoss>cpptresh])
  ...
  eff1=1-sum((dmodelsim-qoss)^2)/sum((qoss-mean(qoss))^2)
  ...
  # Fourth plot
  plot(dmodelsim[aux4],sortmedpredoss,xlim=c(min(min(cbind(dmodelsim[aux4],sortmedpredoss)),0),max(cbind(dmodelsim[aux4],sortmedpredoss))),ylim=c(min(min(cbind(dmodelsim[aux4],sortmedpredoss)),0),max(cbind(dmodelsim[aux4],sortmedpredoss))),xlab="Data simulated by deterministic model",ylab="Observed data",col="red")
  abline(0,1)
  grid()
  #Compute the efficiency of the simulation and put it in the plots
  eff=1-sum((medpred-qoss)^2)/sum((qoss-mean(qoss))^2)
  ...
  cat("Percentage of points lying above the upper confidence limit=",length(qosstemp[qosstemp>suppredtemp[qosstemp>cpptresh]])/length(qosstemp)*100,"%",sep="",fill=T)
  cat("Percentage of points lying below the lower confidence limit=",length(qosstemp1[qosstemp1<infpredtemp[qosstemp>cpptresh]])/length(qosstemp1)*100,"%",sep="",fill=T)
  }
 
 if(is.null(qoss)==F) 
  {
  eff=1-sum((medpred-qoss)^2)/sum((qoss-mean(qoss))^2)
  return(list(detprediction=dmodelsim,stochprediction=medpred,lowlimit=infpred,uplimit=suppred,effsmodel=eff))
  } else
  {
  return(list(detprediction=dmodelsim,stochprediction=medpred,lowlimit=infpred,uplimit=suppred))
  }
}
